...
There are significant differences between procedural and declarative macros in Rust. One of the biggest differences is that procedural macros operate and generate new code at compile time. They are defined as functions that take a `TokenStream` as input and output a `TokenStream`.
